...
class ACMEClient:
    ...
    def loadKey(self, path: str):
        ...
        try:
            ...
            secret = getpass(
                prompt="Enter passphrase for key '{}': ".format(path))
            key = serial.load_ssh_private_key(keyRaw, secret.encode())
        if not isinstance(key, rsa.RSAPrivateKey):
            raise ValueError(
                "Only RSAPrivateKey is supported, got {}".format(type(key)))
        self.keyPriv = key
    ...
    def signAndSend(self, url: str, payload: Optional[JsonDict]):
        ...
        resp = requests.post(url, data=req, headers={
    ...
    def getChallenge(self, authURL: str):
        ...
        if challenge is None:
            raise RuntimeError(
                "No valid challenge found auth={}".format(authURL))
