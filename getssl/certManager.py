...
class CertManager(object):
    ...
    def domain(self, value: str):
        ...
        if value.startswith('*.'):
            self.basePath = os.path.join(
                self.config.wd, 'wildcard', value[2:], self.baseName)
        else:
    ...
    def getCSR(self, keyType: KeyType, keySize: int = ...):
        self.logger("generate CSR domain={}, keySize={}",
                    self._domain, keySize)
        builder = x509.CertificateSigningRequestBuilder()
        builder = builder.subject_name(
            x509.Name([x509.NameAttribute(x509.NameOID.COMMON_NAME, self._domain)]))
    ...
    def savePriv(self, priv: PrivateKeyTypes):
        ...
        privPath = os.path.join(
            self.basePath, '{}-{}'.format(typeName, self.baseName))
        with self.openForSafeWrite(privPath) as fout:
            secret = getpass(
                prompt="Enter passphrase for key '{}': ".format(privPath)).encode()
            encrypt = serial.BestAvailableEncryption(secret)
            privEnc = priv.private_bytes(
                serial.Encoding.PEM, serial.PrivateFormat.TraditionalOpenSSL, encrypt)
            fout.write(privEnc)
        self.logger("save privKey path={}", privPath)

    def saveCSR(self, csr: x509.CertificateSigningRequest, keyType: KeyType):
        csrPath = os.path.join(
            self.basePath, '{}-{}.csr'.format(keyType.value, self.baseName))
    ...
    def saveCert(self, cert: str, keyType: KeyType):
        certPath = os.path.join(
            self.basePath, '{}-{}.ca_bundle'.format(keyType.value, self.baseName))
