...
class GetSSL(Actor):
    ...
    def waitForStatusValid(self, url: str, expect: str, sleep: int = 1, retry: int = 20):
        ...
        while True:
            ...
            if status == expect:
                ...
                raise RuntimeError(
                    "wait object valid failed url={}, status={}, details={}".format(url, status, obj))
            elif retry <= 0:
                raise RuntimeError(
                    "wait object valid timeout url={}, status={}, details={}".format(url, status, obj))
            time.sleep(sleep)
    ...
    def getSSL(self, dnsWait: int = 5):
        ...
        challengeURL, challengeDomain, challengeValue = self.acme.getChallenge(
            authURL)
